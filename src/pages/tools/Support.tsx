...
const stats = [
  ...
  { label: "AI Resolved", value: "156", icon: Sparkles, color: "text-agent-support" },
      ...
      <div className="h-full flex flex-col overflow-hidden">
        ...
        <header className="px-3 md:px-8 py-3 md:py-6 border-b border-border shrink-0">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              <div className="w-9 h-9 md:w-11 md:h-11 rounded-2xl bg-agent-support/10 border border-agent-support/20 flex items-center justify-center overflow-hidden shrink-0">
                <img src={agentSupportLogo} alt="Support" className="w-full h-full object-cover" />
              </div>
              <div className="min-w-0">
                <h1 className="text-lg md:text-2xl font-bold text-foreground leading-tight whitespace-normal break-words">Support Copilot</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-4 mt-3 md:mt-6">
            {stats.map((stat) => (
              <div key={stat.label} className="p-3 md:p-4 rounded-2xl bg-card border border-border flex items-center gap-3 md:gap-4">
                <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-secondary flex items-center justify-center shrink-0">
                  <stat.icon className={`w-4 h-4 md:w-5 md:h-5 ${stat.color}`} />
                </div>
                <div className="min-w-0">
                  <p className="text-base md:text-2xl font-bold text-foreground tabular-nums">{stat.value}</p>
                  <p className="text-[11px] md:text-sm text-muted-foreground leading-tight whitespace-normal break-words">{stat.label}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 p-3 md:p-8 overflow-y-auto">
          ...
          <div className="space-y-2 md:space-y-3">
            {tickets.map((ticket) => (
              <div key={ticket.id} className="p-3 md:p-4 rounded-2xl bg-card border border-border hover:border-agent-support/30 transition-all">
                <div className="flex items-start gap-2 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-medium text-foreground text-sm md:text-base leading-snug whitespace-normal break-words">{ticket.subject}</h3>
                  </div>
                  <div className="text-right shrink-0">
                    <span className={`text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded-full ${
                      ...
                      ticket.status === "ai-response" ? "bg-agent-support/15 text-agent-support" :
