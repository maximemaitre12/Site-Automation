...
export function AgentComplianceDemo({ className }: AgentComplianceDemoProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative p-4 rounded-xl bg-gradient-to-br from-rose-500/5 via-background to-red-500/5 border border-rose-500/20 overflow-hidden",
        className
      )}
    >
      <div className="absolute top-0 right-1/4 w-24 h-24 bg-rose-500/10 rounded-full blur-2xl animate-pulse" />
      <div className="absolute bottom-0 left-1/4 w-20 h-20 bg-red-500/10 rounded-full blur-xl animate-pulse" style={{ animationDelay: "1s" }} />
      ...
      <div className="relative z-10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className={cn(
                      "text-[8px]",
                      isChecked
                        ? item.status === "pass" ? "text-emerald-500" : "text-amber-500"
        ...
        <div className={cn(
          "p-2 rounded-lg bg-amber-500/10 border border-amber-500/20 mb-3 transition-all duration-500",
          phase >= 2 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        )}>
          <div className="flex items-start gap-2">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />
            <div>
              <p className="text-[10px] font-medium text-amber-600 dark:text-amber-400">PII Detected</p>
        ...
        <div className={cn(
          ...
          <div className="text-center p-1.5 rounded bg-rose-500/10">
            <div className="text-sm font-bold text-rose-500">-90%</div>
