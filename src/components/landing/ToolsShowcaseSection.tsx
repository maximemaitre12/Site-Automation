      ...
      <div className="relative z-10 container mx-auto px-6 max-w-7xl">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 mb-4">
            ...
            <span className="text-sm font-medium text-primary">6 outils intégrés</span>
          </div>
          ...
          {tools.map((tool) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-medium hidden sm:inline">{tool.name}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="order-2 lg:order-1">
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${colors.bg} border ${colors.border} mb-4`}>
              <activeTool.icon className={`w-4 h-4 ${colors.text}`} />
              <span className={`text-sm font-medium ${colors.text}`}>{activeTool.tagline}</span>
            </div>
            
            <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-4">{activeTool.name}</h3>
            <p className="text-lg text-muted-foreground mb-8 leading-relaxed">{activeTool.description}</p>
          ...
          <div className="order-1 lg:order-2">
            <div className={`relative rounded-2xl border ${colors.border} bg-card/80 backdrop-blur-xl overflow-hidden`}>
              {/* Header */}
              <div className={`flex items-center justify-between px-4 py-3 border-b ${colors.border} bg-gradient-to-r ${colors.gradient} bg-opacity-10`}>
                <div className="flex items-center gap-2">
                  <activeTool.icon className={`w-5 h-5 ${colors.text}`} />
                  <span className="text-sm font-medium text-foreground">{activeTool.example.title}</span>
                </div>
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full ${colors.bg}`}>
                  <div className={`w-1.5 h-1.5 rounded-full ${colors.text.replace('text-', 'bg-')} animate-pulse`} />
                  <span className={`text-xs font-medium ${colors.text}`}>En cours</span>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  {activeTool.example.steps.map((step, i) => (
                    <div key={i} className="flex flex-col items-center">
                      <div className={`w-12 h-12 rounded-xl ${i < 3 ? 'bg-success/10 border-success/30' : colors.bg + ' ' + colors.border} border flex items-center justify-center mb-2`}>
                        {i < 3 ? (
                      ...
                      <span className="text-xs text-center text-muted-foreground max-w-[70px]">{step}</span>
                      {i < activeTool.example.steps.length - 1 && (
                        <div className="absolute">
                          {/* Connection line handled by flex gap */}
                        </div>
                      )}
                    </div>
                  ))}
            ...
            {/* Glow */}
            <div className={`absolute -inset-4 -z-10 bg-gradient-to-r ${colors.gradient} blur-3xl opacity-20`} />
          </div>
        </div>
      </div>
    </section>
