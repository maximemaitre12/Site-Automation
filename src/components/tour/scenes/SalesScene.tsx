    ...
    >
      ...
      <div className="relative z-10 flex-1 flex gap-[1%] p-[1.5%] overflow-hidden min-h-0">
        ...
        <FadeSlide active={timeline.isActive('pipeline')} direction="left" className="w-[22%] flex flex-col gap-[0.6em]">
          ...
          <StaggerGroup active={timeline.isActive('pipeline')} stagger={100} animation="spring">
            {deals.map((deal, i) => (
              <div 
                key={deal.name}
                className={cn(
                  "p-[0.8em] rounded-xl border transition-all",
              ...
              >
                <div className="flex items-center justify-between mb-[0.3em]">
                  <span className="text-[0.9em] font-medium text-slate-800">{deal.name}</span>
                  {deal.hot && <Target className="w-[0.9em] h-[0.9em] text-amber-500" />}
                </div>
                <div className="text-[0.7em] text-slate-500 mb-[0.5em]">{deal.stage}</div>
        ...
        <div className="flex-1 flex flex-col gap-[0.8em] min-w-0">
          ...
          <SpringIn active={timeline.isActive('callSection')} className="flex-1 rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden min-h-0 flex flex-col">
            ...
            <div className="flex-1 p-[1em] flex gap-[1em] min-h-0">
              {/* Waveform */}
                ...
                <div className="flex flex-col gap-[0.4em]">
                  ...
                  {transcriptLines.map((line, i) => (
                    <FadeSlide 
                      ...
                      direction="left"
                      delay={i * 150}
                      className="p-[0.5em] rounded-lg bg-white border border-slate-100"
                    >
                      <span className={cn("text-[0.75em] font-medium", line.you ? "text-emerald-600" : "text-blue-500")}>
                        {line.speaker}:
                      </span>
                      <span className="text-[0.75em] text-slate-600 ml-[0.3em]">{line.text}</span>
                    </FadeSlide>
                  ))}
                </div>
              </ScanLine>
            ...
            <SpringIn active={timeline.isActive('analysis')} className="flex-1 rounded-xl bg-slate-50 border border-slate-100 p-[0.8em] flex flex-col">
              ...
              <StaggerGroup active={timeline.isActive('metrics')} stagger={80} className="flex-1 grid grid-cols-4 gap-[0.5em]">
                {analysisMetrics.map((metric) => (
                  <div key={metric.label} className="p-[0.5em] rounded-lg bg-white border border-slate-100 flex flex-col items-center justify-center">
                    ...
                    <div className="text-[0.6em] text-slate-500 mt-[0.2em]">{metric.label}</div>
                  </div>
                ))}
              </StaggerGroup>
            </SpringIn>
        ...
        <FadeSlide active={timeline.isActive('team')} direction="right" className="w-[20%] flex flex-col gap-[0.8em]">
          {/* Team performance */}
              ...
              {teamMembers.map((member) => (
                <div key={member.name}>
                  <div className="flex items-center justify-between mb-[0.2em]">
                    <span className="text-[0.7em] text-slate-600">{member.name}</span>
                    <span className={cn("text-[0.7em] font-semibold", member.quota >= 100 ? "text-emerald-500" : "text-slate-500")}>
