...
// Professional timeline with zoom phases
const hrTimeline: Timeline = {
  ...
  zoomToCV: { start: 30, end: 55 }, // Zoom to CV analysis
  candidates: { start: 38, end: 55, stagger: 150, items: 3 },
  matchScores: { start: 48, end: 62 },
  zoomToBottom: { start: 58, end: 78 }, // Zoom to bottom row
...
export function HRScene({ isActive, progress }: HRSceneProps) {
  ...
  const getZoomTarget = () => {
    if (timeline.isActive('zoomToCV') && !timeline.isActive('zoomToBottom')) return 'top-center';
    if (timeline.isActive('zoomToBottom')) return 'bottom-center';
    return 'center';
  };

  const isZooming = timeline.isActive('zoomToCV') || timeline.isActive('zoomToBottom');
  ...
  return (
    ...
    >
      ...
      <ZoomSpotlight active={isZooming && !timeline.isActive('stats')} targetArea={getZoomTarget()} zoomLevel={1.12}>
            ...
            <FadeSlide active={timeline.isActive('pipeline')} direction="left" className="w-[22%] flex flex-col gap-[0.6em] min-w-0">
              ...
              <StaggerGroup active={timeline.isActive('pipeline')} stagger={120} animation="spring" className="flex flex-col gap-[0.5em]">
                {pipelineStages.map((stage, i) => (
                  <div 
                    key={stage.label}
                    className="relative p-[0.8em] rounded-xl bg-slate-50 border border-slate-100 hover:border-slate-200 transition-colors overflow-hidden"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col gap-[0.5em]">
                      {candidates.map((candidate, i) => (
                        <FadeSlide 
                          ...
                          delay={i * 150}
                          className={cn(
                            "p-[0.6em] rounded-xl border transition-all duration-300 overflow-hidden",
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-[0.6em] min-w-0 flex-1">
                              <div className="w-[1.8em] h-[1.8em] rounded-full bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white font-medium text-[0.7em] shrink-0">
                                {candidate.name[0]}
                              </div>
                          ...
                          <div className="mt-[0.4em] flex gap-[0.3em] overflow-hidden">
                            {candidate.skills.slice(0, 2).map(skill => (
                              <span key={skill} className="px-[0.5em] py-[0.15em] rounded-full bg-slate-100 text-[0.65em] text-slate-600 truncate">
                                {skill}
                              </span>
                            ))}
                          </div>
                ...
                <FadeSlide active={timeline.isActive('emails')} direction="up" className="flex-1 rounded-xl bg-slate-50 border border-slate-100 p-[0.8em] flex flex-col overflow-hidden">
                  ...
                  <StaggerGroup active={timeline.isActive('emails')} stagger={80} className="flex-1 flex gap-[0.5em]">
                    {[
                      ...
                      { icon: CheckCircle, label: 'Confirms', color: 'text-emerald-500' },
                    ].map((email) => (
                      <div 
                        key={email.label}
                ...
                <StaggerGroup active={timeline.isActive('interviews')} stagger={100} animation="slide" className="flex-1 flex flex-col gap-[0.4em] overflow-hidden">
                  {interviews.map((interview) => (
                    <div 
                      key={interview.time}
                      className={cn(
                        "p-[0.5em] rounded-lg border transition-all overflow-hidden",
                        interview.live 
                  ...
                  <div className="space-y-[0.4em]">
                    {[
                      ...
                      <div key={stat.label} className="flex items-center justify-between">
                        ...
                        <span className={cn(
                          "text-[0.85em] font-bold transition-colors duration-500 shrink-0",
